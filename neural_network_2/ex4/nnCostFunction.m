...
z2 = actv1*Theta1';
actv2 = sigmoid(z2);
actv2 = [ones(m,1) actv2];
z3 = actv2*Theta2';
actv3 = sigmoid(z3);
h_x = actv3;

% step below for inilize ys as vector for each element of y if y(1) = 10 then
% you should off all of vector in index 1 to zero except tem elements to 1
% 
%
...
for k=1:num_labels
	ys = y==k;
	hs_one_label = h_x(:, k);
	er = (((-ys.*log(hs_one_label) - (1-ys).*(log(1-hs_one_label)))));
	J = J + sum(er);
end
J = (1/m)*J;
